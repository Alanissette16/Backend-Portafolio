...
public interface AsesoriaRepository extends JpaRepository<AsesoriaEntity, Long> {
    ...
    // Busca asesorías por ID del programador con paginación
    Page<AsesoriaEntity> findByProgramadorId(Long programadorId, Pageable pageable);

    // Busca asesorías por ID del usuario externo (solicitante)
    List<AsesoriaEntity> findByUsuarioExternoId(Long usuarioExternoId);

    // Busca asesorías por ID del usuario externo con paginación
    Page<AsesoriaEntity> findByUsuarioExternoId(Long usuarioExternoId, Pageable pageable);
